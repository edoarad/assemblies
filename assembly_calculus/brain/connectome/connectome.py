...
class Connectome(ABCConnectome):
	...
	def update_connectomes(self, new_winners: Dict[Area, List[int]], sources: Dict[Area, List[BrainPart]]) -> None:
		...
		for area in new_winners:
			for source in sources[area]:
				...
				self.connections[source, area].synapses[source_neurons, new_winners[area][:]] *= (1 + beta)
	...
	def project_into(self, area: Area, sources: List[BrainPart]) -> List[int]:
		...
		for source in src_areas:
			...
			prev_winner_inputs += np.sum((area_connectomes.synapses[winner, :] for winner in self.winners[source]),
			                             axis=0)
		if src_stimuli:
	...
	# TODO: change name
	def project(self, connections: Dict[BrainPart, List[Area]], *, override_winners: Dict[Area, List[int]] = None,
